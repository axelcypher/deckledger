...
function renderCardModal(){
  ...
  $('#card-dialog').innerHTML=`<div class="card-modal-layout">
    ...
    <section class="card-stage"><div class="stage-backdrop"></div><div class="variant-hint top">${idx>0?`<button data-variant-nav="-1">↑ ${escapeHtml(variantLabel(physicalVariants[idx-1]))}</button>`:''}</div><button class="card-nav-button prev" ${gridIdx<=0?'disabled':''} data-card-nav="-1">‹</button><div class="modal-card-frame card-finish-frame ${visual.effect}"><img class="modal-card-image" src="${artUrl(v.id,'full')}" alt="${escapeHtml(card.canonical_name)}"><img class="modal-card-reflection" src="${artUrl(v.id,'full')}" alt="" aria-hidden="true"></div><button class="card-nav-button next" ${gridIdx<0||gridIdx>=state.cards.length-1?'disabled':''} data-card-nav="1">›</button><div class="variant-hint bottom">${idx<physicalVariants.length-1?`<button data-variant-nav="1">↓ ${escapeHtml(variantLabel(physicalVariants[idx+1]))}</button>`:''}</div></section>
    <aside class="modal-side"><header class="modal-head"><span class="eyebrow">${escapeHtml(v.set_code)} · ${escapeHtml(v.collector_number)}</span><h2>${escapeHtml(card.canonical_name)}</h2><b class="modal-price">${price(v.price)}</b><span class="modal-set-line">${escapeHtml(v.set_code)} · ${escapeHtml(v.collector_number)}</span><span class="modal-rarity-line">${escapeHtml(v.rarity)} · ${escapeHtml(variantName(v))}</span><p>${escapeHtml(v.rarity)} · ${escapeHtml(variantName(v))}</p><div class="language-switcher" aria-label="Sprachversion">${languages.map(language=>`<button data-language="${language}" class="${language===v.language?'active':''}">${language}</button>`).join('')}</div></header><nav class="modal-tabs">${[['collection','Sammlung'],['market','Markt'],['card','Karte'],['relationships','Beziehungen']].map(([key,label])=>`<button data-tab="${key}" class="${state.modalTab===key?'active':''}">${label}</button>`).join('')}</nav><div class="modal-content">${modalTabContent(card,v)}</div></aside>
